...
OPENAI_POSTER_ANALYSIS_MODEL=gpt-5.6-luna
...
- 海报模式的沙发分析、自由创意策划和文字校验默认通过 `http://192.168.50.70:8888` 调用 `gpt-5.6-luna`，完整带字海报调用 `gpt-image-2`；地址和模型分别由 `OPENAI_API_BASE_URL`、`OPENAI_POSTER_ANALYSIS_MODEL`、`OPENAI_POSTER_IMAGE_MODEL` 配置。
