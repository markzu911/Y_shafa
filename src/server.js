...
async function handleGenerate(req, res) {
  ...
  const prompt = [
    ...
    '模特图规则：如果当前选择的是“模特”场景，模特必须真实坐在沙发上，身体重量要落在坐垫上，臀部、大腿和沙发坐面之间要有明确接触关系，姿态要符合坐姿，不能站在旁边、靠在旁边、坐在扶手上、漂浮在沙发上方，或者只是出现在沙发附近。',
    '生成原则：绝对不要把用户上传的房间图片当作底图直接修改、覆盖、局部涂抹或贴入沙发。房间图片只用于分析空间布局、家具关系、装修风格、材质、采光和相机视角；最终效果图必须根据这些分析结果重新生成一个一致的房间场景，再把沙发自然融入进去。',
